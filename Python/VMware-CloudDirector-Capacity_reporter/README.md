...
## Назначение
Предназначен для получения консолидированной информации о ресурсах **Provider VDC** в VMware Cloud Director.
Скрипт формирует отчет по следующим категориям данных:
1. Служебная информация
- статус выполнения;
- версия API;
- признак проверки SSL-сертификата.

2. Информация о Provider VDC
- связанный `vimServer`;
- связанный `nsxTManager`.
...
- CPU used;
- CPU overhead;
- CPU total reservation;
- Memory allocation;
- Memory reserved;
- Memory total;
- Memory used;
- Memory overhead;
- Memory total reservation.

4. Конвертированные значения
Для удобства чтения скрипт дополнительно рассчитывает:
- CPU в MHz, GHz и THz;
- Memory в MB, GB и TB;
- Storage в MB, GB и TB.

5. Ratios и pressure-метрики
Рассчитает коэффициенты:
...
- `memory_usage_ratio`;
- usage ratio по storage policy;
- общий usage ratio по datastore.

6. Агрегирование storage by policy
Агрегирует данные по `adminOrgVdcStorageProfile` и показывает:
- имя storage policy;
- количество Org VDC profiles;
- суммарный used storage;
- суммарный storage limit;
- usage ratio по каждой политике;
- общий summary по всем storage policy.

7. Datastore capacity
Агрегирует данные по `datastore` и рассчитывает:
- количество datastore;
- суммарную полную ёмкость;
- суммарно занятую ёмкость;
- provisioned storage;
- requested storage;
- коэффициенты использования и переподписки.
